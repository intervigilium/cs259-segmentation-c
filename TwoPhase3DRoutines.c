	...
	for (i = 0; i < gridData->m; i++) {
		for (j = 0; j < gridData->n; j++) {
			for (k = 0; k < gridData->p; k++) {
				xx = (double)(i) * gridData->dx +
				    gridData->xMin;
				yy = (double)(j) * gridData->dy +
				    gridData->yMin;
				zz = (double)(k) * gridData->dz +
				    gridData->zMin;
				PPHI(i, j, k) =
				    sqrt((xx - xcent) * (xx - xcent) +
					 (yy - ycent) * (yy - ycent) + (zz -
									zcent) *
					 (zz - zcent))
				    - r;
			}
...
//#####################################################################
//
void getMeanIntensities1(const double u0[M][N][P],
	...
	for (i = 1; i < m - 1; i++) {
		for (j = 1; j < n - 1; j++) {
			for (k = 1; k < p - 1; k++) {
				...
				C1x =
				    1.0 / sqrt(Dx_p * Dx_p + Dy_0 * Dy_0 +
					       Dz_0 * Dz_0 + eps_sqrd);
				C2x =
				    1.0 / sqrt(Dx_m * Dx_m + Dy_0 * Dy_0 +
					       Dz_0 * Dz_0 + eps_sqrd);
				C3y =
				    1.0 / sqrt(Dx_0 * Dx_0 + Dy_p * Dy_p +
					       Dz_0 * Dz_0 + eps_sqrd);
				C4y =
				    1.0 / sqrt(Dx_0 * Dx_0 + Dy_m * Dy_m +
					       Dz_0 * Dz_0 + eps_sqrd);
				C5z =
				    1.0 / sqrt(Dx_0 * Dx_0 + Dy_0 * Dy_0 +
					       Dz_p * Dz_p + eps_sqrd);
				C6z =
				    1.0 / sqrt(Dx_0 * Dx_0 + Dy_0 * Dy_0 +
					       Dz_m * Dz_m + eps_sqrd);
				...
				MM = (dt / (dx * dy)) * Grad * mu;
				CC = 1 + M * (C1x + C2x + C3y + C4y + C5z +
					      C6z);

				C1x_2x =
				    C1x * PPHI(i + 1, j, k) + C2x * PPHI(i - 1,
									 j, k);
				C3y_4y =
				    C3y * PPHI(i, j + 1, k) + C4y * PPHI(i,
									 j - 1,
									 k);
				C5z_6z =
				    C5z * PPHI(i, j, k + 1) + C6z * PPHI(i, j,
									 k - 1);
	...
	for (i = 1; i < m - 1; i++) {
		for (j = 1; j < n - 1; j++) {
			for (k = 1; k < p - 1; k++) {
				...
				Dxz =
				    (PPHI(i + 1, j, k + 1) -
				     PPHI(i + 1, j, k - 1) - PPHI(i - 1, j,
								  k + 1) +
				     PPHI(i - 1, j, k - 1)) / (4 * dx * dz);
				Dyz =
				    (PPHI(i, j + 1, k + 1) -
				     PPHI(i, j + 1, k - 1) - PPHI(i, j - 1,
								  k + 1) +
				     PPHI(i, j - 1, k - 1)) / (4 * dy * dz);

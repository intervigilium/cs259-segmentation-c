			     ...
			     TwoPhase3D * AC)
{
	...
	for (j = 1; j < n - 1; j++) {
		...
		C1x =
		    1.0 / sqrt(Dx_p * Dx_p + Dy_0 * Dy_0 + Dz_0 * Dz_0 +
			       eps_sqrd);
		C2x =
		    1.0 / sqrt(Dx_m * Dx_m + Dy_0 * Dy_0 + Dz_0 * Dz_0 +
			       eps_sqrd);
		C3y =
		    1.0 / sqrt(Dx_0 * Dx_0 + Dy_p * Dy_p + Dz_0 * Dz_0 +
			       eps_sqrd);
		C4y =
		    1.0 / sqrt(Dx_0 * Dx_0 + Dy_m * Dy_m + Dz_0 * Dz_0 +
			       eps_sqrd);
		C5z =
		    1.0 / sqrt(Dx_0 * Dx_0 + Dy_0 * Dy_0 + Dz_p * Dz_p +
			       eps_sqrd);
		C6z =
		    1.0 / sqrt(Dx_0 * Dx_0 + Dy_0 * Dy_0 + Dz_m * Dz_m +
			       eps_sqrd);
				  ...
				  TwoPhase3D * AC, double dt)
{
	...
	for (j = 1; j < n - 1; j++) {
		...
		Dxy =
		    (PPHI(i + 1, j + 1, k) - PPHI(i + 1, j - 1, k) -
		     PPHI(i - 1, j + 1, k) + PPHI(i - 1, j - 1,
						  k)) / (4 * dx * dy);
		Dxz =
		    (PPHI(i + 1, j, k + 1) - PPHI(i + 1, j, k - 1) -
		     PPHI(i - 1, j, k + 1) + PPHI(i - 1, j,
						  k - 1)) / (4 * dx * dz);
		Dyz =
		    (PPHI(i, j + 1, k + 1) - PPHI(i, j + 1, k - 1) -

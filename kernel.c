				  ...
				  double c1, double c2)
{
		...
		for (j = 1; j < n - 1; j++) {
			for (k = 1; k < p - 1; k++) {
				...
				K = (Dx_0 * Dx_0 * Dyy -
				    ...
				    /
				    (pow
				     (Dx_0 * Dx_0 + Dy_0 * Dy_0 + Dz_0 * Dz_0,
				      1.5) + runData->epsilon);

				if (AC->grad == 'g') {
					Grad =
					    sqrt(Dx_0 * Dx_0 + Dy_0 * Dy_0 +
						 Dz_0 * Dz_0);
				} else if (AC->grad == 'd') {
	...
	for (j = 0; j < N; j++) {
		for (k = 0; k < P; k++) {
			curvature_motion_part[0][j][k] =
			    curvature_motion_part[1][j][k];
			curvature_motion_part[M - 1][j][k] =
			    curvature_motion_part[M - 2][j][k];
		}
	}

	for (i = 0; i < M; i++) {
		for (k = 0; k < P; k++) {
			curvature_motion_part[i][0][k] =
			    curvature_motion_part[i][1][k];
			curvature_motion_part[i][N - 1][k] =
			    curvature_motion_part[i][N - 2][k];
		}
	}

	for (i = 0; i < M; i++) {
		for (j = 0; j < N; j++) {
			curvature_motion_part[i][j][0] =
			    curvature_motion_part[i][j][1];
			curvature_motion_part[i][j][P - 1] =
			    curvature_motion_part[i][j][P - 2];
		}
	}
	...
	curvature_motion_part[M - 1][N - 1][0] =
	    curvature_motion_part[M - 2][N - 2][1];
	curvature_motion_part[M - 1][0][P - 1] =
	    curvature_motion_part[M - 2][1][P - 2];
	curvature_motion_part[0][N - 1][P - 1] =
	    curvature_motion_part[1][N - 2][P - 2];
	curvature_motion_part[M - 1][N - 1][P - 1] =
	    curvature_motion_part[M - 2][N - 2][P - 2];
	...
	for (i = 0; i < M; i++) {
		for (j = 0; j < N; j++) {
			for (k = 0; k < P; k++) {
				phi[i][j][k] +=
				    curvature_motion_part[i][j][k] * dt;
			}
		}
	}
}
